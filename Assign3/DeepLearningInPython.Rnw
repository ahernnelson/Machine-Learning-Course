...
<<include=FALSE>>=
library(reticulate)
use_condaenv('C:/Users/PACMAN/Anaconda3/python')
@
...
hidden_layers = np.array([node_0_val,node_1_val])
output = hidden_layers*weights['output'].sum()
print(output)

#### Instead using activation function
def relu(input):
    output = max(0, input)
    return(output)
